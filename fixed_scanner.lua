...
local function initConfigFolder()
    ...
    if not success then
        warn("Config folder creation failed: " .. tostring(error))
        return false
    end
    return true
end
...
local function loadConfig()
    ...
    local success, result = pcall(function()
        ...
        if not configData or configData == "" then
            print("📁 Config file empty, using defaults")
            return defaultConfig
        end
    ...
    if success and result then
        ...
        warn("⚠️ Config load failed: " .. tostring(result))
        return defaultConfig
    end
end
...
local function saveConfig()
    ...
    if not success then
        warn("⚠️ Config save failed: " .. tostring(error))
        return false
    end
    return true
end
...
local function applyConfig(config)
    ...
    print("📁 Applying config type: " .. type(config))
    ...
    if displayConfig and type(displayConfig) == "table" then
        ...
        print("📁 Applied display settings from config")
    else
    ...
    if rarityConfig and type(rarityConfig) == "table" then
        ...
        print("📁 Applied rarity settings from config")
    else
        raritySettings["Secret"].Enabled = true
        raritySettings["Brainrot God"].Enabled = true
        print("📁 Using default rarity settings")
    end
    ...
    if petsConfig and type(petsConfig) == "table" then
        ...
        print("📁 Applied pet settings from config")
    else
        for petName, petData in pairs(specificBrainrots) do
    ...
    print("📁 Config successfully applied to all settings")
end
...
local function createAutoHopToggle()
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        if autoHopEnabled then
            print("🔄 Auto Hop Enabled - Will automatically hop servers using hybrid regional hopper")
        else
            print("🔄 Auto Hop Disabled - Will stay in current server")
        end
...
local function createNotificationGUI()
    ...
    local function createSettingsContent()
        ...
        hopButton.MouseButton1Click:Connect(function()
            ...
            if isHopping then
                print("🔄 Already hopping servers, please wait...")
                return
            end
            
            isHopping = true
            print("🏠 Force manual hop initiated...")
            ...
            if success then
                print("✅ Manual-style teleport initiated!")
            else
                print("❌ Manual teleport failed: " .. tostring(err))
                isHopping = false
            end
        end)
    ...
    if shouldCheckWebhook then
        ...
        if #newPets > 0 then
            ...
            if autoHopEnabled and tick() - scanStartTime >= SCAN_TIMEOUT then
                print("🔄 No pets found after scanning - hopping servers...")
                scanStartTime = 0
                scanCompleted = false
                task.wait(hopDelay)
                _G.hopServer()
...
local function checkPlayerChanges()
    ...
    for userId, name in pairs(currentPlayers) do
        if not lastPlayerList[userId] then
            print("👋 New player joined: " .. name .. " - Will scan for their pets...")
    ...
    for userId, name in pairs(lastPlayerList) do
        if not currentPlayers[userId] then
            print("👋 Player left: " .. name .. " - Cleaning up their pets...")
...
local function tryRegionalHop()
    ...
    print("🌍 Attempting regional hop to: " .. selectedRegion)
    ...
    if not success then
        print("❌ Regional request failed, falling back to manual-style")
        return false
    end
    
    if not serverData.data or #serverData.data == 0 then
        print("❌ No servers found in " .. selectedRegion .. ", falling back to manual-style")
        return false
    end
    ...
    if #availableServers == 0 then
        print("❌ No suitable servers in " .. selectedRegion .. ", falling back to manual-style")
        return false
    end
    
    -- Pick random server from available ones
    local targetServer = availableServers[math.random(1, #availableServers)]
    print(string.format("🎯 Targeting %s server: %s (%d/%d players)",
    ...
    if teleportSuccess then
        print("✅ Regional teleport initiated to " .. selectedRegion)
        return true
    else
        print("❌ Regional teleport failed: " .. tostring(teleportError) .. ", falling back to manual-style")
        return false
    end
end
...
_G.hopServer = function()
    ...
    isHopping = true
    retryCount = 0
    print("🌍 Starting hybrid server hopper (Regional -> Manual fallback)...")
    ...
    if not regionalSuccess then
        ...
        print("🏠 Falling back to manual-style teleport...")
        local TeleportService = game:GetService("TeleportService")
        
        local function tryManualHop()
            retryCount = retryCount + 1
            print(string.format("🔄 Manual attempt #%d...", retryCount))
            ...
            task.spawn(function()
                ...
                if isHopping and not timeout then
                    timeout = true
                    print("⏰ Manual teleport timeout - Will retry...")
                    task.wait(2)
                    tryManualHop()
                end
            end)
            ...
            local teleportFailConnection = TeleportService.TeleportInitFailed:Connect(function(player, teleportResult, errorMessage)
                if player == LocalPlayer and isHopping and not timeout then
                    print(string.format("❌ Manual teleport failed: %s - Will retry...", tostring(teleportResult)))
            ...
            if success then
                ...
                if not timeout then
                    print("❌ Manual teleport call failed: " .. tostring(err))
    ...
    task.spawn(function()
        ...
        if isHopping then
            print("🔧 Backup cleanup: Resetting hopping flag")
            isHopping = false
        end
    end)
end
    ...
    local function onCharacterAdded(character)
        if isHopping then
            ...
            if currentJobId and currentJobId ~= "" then
                print("✅ Successfully hopped to new server!")
                isHopping = false
                retryCount = 0
                ...
                print("🔄 Still loading new server...")
            end
        end
    end
...
local function startScanner()
    ...
    print("🚀 HYBRID Server hopper available (Regional + Manual fallback)")
    print("🔄 Auto server hop: " .. (autoHopEnabled and "ENABLED (HYBRID)" or "DISABLED"))
    print("📡 Webhook notifications: "..(DISCORD_WEBHOOK_URL ~= "" and "ENABLED" or "DISABLED"))
    print("⚡ ENHANCED: Regional variety with manual-style reliability!")
    print("🌍 Regions: Japan, Korea, Brazil, Spain, EU, Netherlands, Ukraine, Vietnam, USA, Canada, UK, Germany, France, Australia, Singapore, India, Mexico, Turkey")
end
...
_G.toggleRarity = function(rarity, enabled)
    if raritySettings[rarity] then
        ...
        print("🔧 " .. rarity .. " rarity " .. (enabled and "enabled" or "disabled"))
        clearAndRefreshUI()
    else
...
_G.showBlacklist = function()
    ...
    for id, data in pairs(blacklistedServers) do
        print(string.format(" %s: %s (%.1f min ago)", id, data.reason, (tick() - data.timestamp) / 60))
    end
end

_G.clearBlacklist = function()
    blacklistedServers = {}
    print("🔧 Server blacklist cleared")
end

_G.getHopStatus = function()
    print(string.format("🔄 Hop Status: %s | Retry Count: %d | Blacklisted: %d",
        isHopping and "HOPPING" or "IDLE", retryCount, #blacklistedServers))
end

_G.clearReported = function()
    reportedPets = {}
    print("🔧 Cleared reported pets - webhooks will trigger again")
end
